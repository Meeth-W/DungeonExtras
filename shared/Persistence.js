...
export class Persistence {
    static data = new PogObject("DungeonExtras", {
     ...
     * @param {string} filePath The relative path of where it is located in DungeonExtras/data
     * @param {*} defaultValue The default value to return if the data is not found (e.g [] or {})
     * @returns {*} 
     */
    static getDataFromFile(filePath, defaultValue = {}) {
        return JSON.parse(FileLib.read("DungeonExtras", `data/${filePath}`)) ?? defaultValue
    }
    ...
    static getDataFromFileOrLink(filePath, url, defaultValue = {}) {
        const thefile = FileLib.read("DungeonExtras", `data/${filePath}`)
    ...
    static saveDataToFile(filePath, data = {}, createFolderTree = true) {
        FileLib.write("DungeonExtras", `data/${filePath}`, JSON.stringify(data, null, 4), createFolderTree)
    }
