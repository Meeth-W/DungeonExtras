...
const highlightSpot = () => {
    ...
    entityList.forEach(obj => {
        ...
        if (time) {
            ...
            const color = fixedTime < 1.5 ? Renderer.RED : Renderer.GREEN

            if (fixedTime < parseFloat(config.autoSwingTime) && fixedTime > (parseFloat(config.autoSwingTime) - 0.05) && mobsSpawned > 3) { swing = true }
...
//             swing = false
//             leftClick()
//             ChatLib.chat(prefix + "Swinging!")
//         }, 50)
//     }
// }), config.autoSwing && config.bloodHelper)
...
const autoSwing = () => {
    if (swing && !swinging) {
        ...
        setTimeout(() => {
            ...
            ChatLib.chat(prefix + `&aSwinging! &6(${mobsSpawned}/19)&a.`)
            if (config.partyProgress) ChatLib.command(`pc Blood: ${mobsSpawned}/19`)
...
new Event(feature, "tick", autoSwing, () => WorldState.inDungeons() && inBlood && config.autoSwing && config.bloodHelper)
