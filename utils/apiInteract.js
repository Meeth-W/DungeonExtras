...
const spcmds = ["nw", "bank", "auctions", "sblvl", "skills", "skillaverage", "overflowskills", "overflowskillaverage", "bestiary",
...
const makeTxtRequest = (address) => request({
    url: address,
    headers: {
        'User-Agent': ' Mozilla/5.0',
        'Content-Type': 'application/json'
...
export function spcmd(cmd, username) {
    if (!spcmds.includes(cmd)) return ChatLib.chat(`${prefix} Command not found.`)
    chatid(`${prefix} &fGetting &6${username} &fSoopy Data...`, 1515)
    Promise.resolve(makeTxtRequest(`https://soopy.dev/api/soopyv2/botcommand?m=${cmd}&u=${username}`)).then(msg => {
...
export const getPlayerUuid = (username, out) => {
